...
\lstset{
	...
	language=TeX,
	morekeywords={\documentclass,\begin,\end,\usepackage,\makeinvoice,\setinvoicetitle,\setinvoicenumber,\setreceivername,\setreceiveraddress,\setname,\setaddress,\setphonenumber,\setemail,\setyourref,\setourref,\setinvoicedate,\setdeadline,\additem,\setsubtotal,\setvat,\settotal,\setaccountnumber, \setsummary, \setcurrancy, \setvattext, \setaccounttext},
...
\subsection{{\textbackslash}setcurrancy} \label{par:setcurrancy}
Specify a currency to use in all price fields of the invoice. This command is optional, the default currency is ``USD''. Takes one argument: the name of the tax use. Example:
\begin{lstlisting}
\setcurrancy{CAD}
\end{lstlisting}
...
Specify custom text for the VAT labels. This command is optional, the default text is ``VAT''. Takes one argument: the currency code to use. Example:
\begin{lstlisting}
\setvattext{HST}
\end{lstlisting}

...
\lstset{
	...
	language=TeX,
	morekeywords={\documentclass,\begin,\end,\usepackage,\makeinvoice,\setinvoicetitle,\setinvoicenumber,\setreceivername,\setreceiveraddress,\setname,\setaddress,\setphonenumber,\setemail,\setyourref,\setourref,\setinvoicedate,\setdeadline,\setdeadlinedate,\additem,\setsubtotal,\setvat,\settotal,\setdisclaimer,\setpayee,\setaccountnumber,\setrouting},
...
\begin{itemize}
	...
	\item \nameref{par:setdeadlinedate}
	\item \nameref{par:additem}
	\item \nameref{par:setsubtotal}
	\item \nameref{par:setvat}
	\item \nameref{par:settotal}
	\item \nameref{par:setdisclaimer}
	\item \nameref{par:setpayee}
	\item \nameref{par:setaccountnumber}
	\item \nameref{par:setrouting}
...
\subsection{{\textbackslash}setdeadlinedate} \label{par:setdeadlinedate}
Specify the deadline of the invoice precisely. This command is optional, and the deadline is set using \texttt{\textbackslash setdeadline} logic if the command is omitted. Overrides \texttt{\textbackslash setdeadline} when command is used. Takes one argument: the precise deadline date. Example:
\begin{lstlisting}
\setdeadlinedate{March 10, 2024}
\end{lstlisting}
...
\subsection{{\textbackslash}setdisclaimer} \label{par:setdisclaimer}
Changes the default warning text about late payments. This command is optional, and the warning text will be the default message if the command is omitted. Takes one agument: the warning text. Example:
\begin{lstlisting}
\setdisclaimer{Late payments may cause delays in shipment}
\end{lstlisting}

\subsection{{\textbackslash}setpayee} \label{par:setpayee}
Specify whom the bank transfer should be made out to. This command is optional, and no payee line will appear in payment details if the command is omitted. Takes one argument: the person or company name. Example:
\begin{lstlisting}
\setpayee{McNameface LLC}
\end{lstlisting}

\subsection{{\textbackslash}setaccountnumber} \label{par:setaccountnumber}
Specify the bank account number for payment. This command is optional, and no account number line will appear in payment details if the command is omitted. Takes one argument: the bank account number for payment. Example:
\begin{lstlisting}
\setaccountnumber{1234 56 78910}
\end{lstlisting}

\subsection{{\textbackslash}setrouting} \label{par:setrouting}
Specify the bank routing number for payment. This command is optional, and no routing number line will appear in payment details if the command is omitted. Takes one argument: the bank routing number for payment. Example:
\begin{lstlisting}
\setrouting{99888777}
\end{lstlisting}
